...
/* ── sys_tkill(tid, sig) — EAX=238 ───────────────────────────────────────────
 * musl's pthread_kill() is tkill(tid, sig) (glibc uses tgkill).  Same delivery
 * as tgkill to the thread's own group. */
static int sys_tkill(registers_t *regs) {
    ...
    for (int i = 0; i < MAX_PROCS; i++) {
        if (ptable[i].state == PROC_UNUSED || ptable[i].pid != tid) continue;
        if (sig) signal_send(&ptable[i], sig);
        return 0;
    }
    return -3;  /* -ESRCH */
}
