 ...
 */
#define PROBE_NAME "p24_unlink_open"
#include "probe.h"
#include <sys/stat.h>
...
static void churn_alloc(const char *tag)
{
    for (int i = 0; i < CHURN; i++) {
        snprintf(churn_path[i], sizeof churn_path[i],
                 "/tmp/p24.%s.%d.%d", tag, (int)getpid(), i);
        unlink(churn_path[i]);
...
int main(void)
{
    ...
    snprintf(path, sizeof path, "/tmp/p24.open.%d", (int)getpid());
    unlink(path);
    ...
    snprintf(path, sizeof path, "/tmp/p24.share.%d", (int)getpid());
    unlink(path);
    ...
    snprintf(dir, sizeof dir, "/tmp/p24.dir.%d", (int)getpid());
